...
`JSON.stringify` and `JSON.parse` are expected to be handled outside of the scope of Guldencore methods. For example, calling `JSON.stringify` on a Guldencore object will behave as expected and call `transaction.toJSON()` and then stringify it:
